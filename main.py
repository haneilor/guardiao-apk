...
def solicitar_permissoes_android():
    try:
        ...
        print("✅ Permissões solicitadas ao Android.")
    except ImportError:
        pass
...
def pegar_localizacao_e_bateria():
    ...
    # 1. Tenta via Hardware Nativo Android (Compilado em APK)
    try:
...
def iniciar():
    ...
    return jsonify({"ok": True, "mensagem": "🟢 Guardião IA Iniciado!"})

@app.route('/parar', methods=['POST'])
def parar():
    global monitoramento_ativo
    monitoramento_ativo = False
    return jsonify({"ok": True, "mensagem": "⏸️ Monitoramento suspenso."})

@app.route('/panico', methods=['POST'])
def panico():
    lat, lon, _ = pegar_localizacao_e_bateria()
    localizacao_str = f"{lat:.6f},{lon:.6f}"
    alertas.enviar_alerta("BOTAO DE PANICO PRESSIONADO!", localizacao_str, nivel=3)
    return jsonify({"ok": True, "mensagem": "🚨 ALERTA DE PÂNICO DISPARADO!"})
