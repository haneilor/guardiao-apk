...
# --- CONFIGURAÇÃO DE DIRETÓRIO PRIVADO SEGURO NO ANDROID ---
...
class GerenciadorAlertas:
    ...
    def verificar_seguro(self, lat_atual, lon_atual):
        ...
        for nome, lat_local, lon_local in locais:
            ...
            if dist_metros <= 50:
                return True, f"Imunizado em área protegida: {nome}"
        return False, "Fora de área de geofencing"
    ...
    def enviar_alerta(self, nivel, mensagem, localizacao):
        ...
        link = f"http://maps.google.com/?q={localizacao}"
        texto = f"GUARDIAO {titulo}: {mensagem}. Localizacao: {link}"

        enviado_via_android = False
        # Disparo nativo usando a API de Hardware SMS do Android via Pyjnius
        try:
        ...
        if not enviado_via_android:
            print(f"\n📡 [SIMULADOR DE BACKUP] SMS Gerado no Log:\nTexto: {texto}")
            for id_c, nome, tel in contatos:
