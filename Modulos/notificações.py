...
class GerenciadorAlertas:
    ...
    def enviar_alerta(self, nivel, mensagem, localizacao):
        ...
        link = f"https://maps.google.com/?q={localizacao}"
        ...
        if not enviado_via_android:
            try:
                ...
                print("🚀 [CHIP TERMUX] Disparado via Hardware do Chip através do subsistema Termux-API!")
            except Exception as e:
                print(f"\n📡 [SIMULADOR DE CHIP] SMS Corporificado:\nTexto: {texto}")
                for id_c, nome, tel in contatos:
                    print(f" -> Canal de Envio: {nome} (+55{self._formatar_telefone(tel)})")
        return True, "✅ Alarmes disparados via CHIP com sucesso!"
